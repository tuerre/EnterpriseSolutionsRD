...
const deleteDepartment = async (req, res) => {
	try {
		...
		const user_id = Number(req.user?.user_id);
		const userValidation = await validateAuthenticatedUser(req, 'delete department');
		...
		if (existingDepartment.is_active === false) {
			return res.status(400).json({ error: 'The department is already inactive' });
		}
		...
		await createSystemMovement({
			...
			actionType: 'ELIMINAR_DEPARTAMENTO',
			description: `Desactivó el departamento "${updatedDepartment.name}"`
		});

		return res.status(200).json({ message: 'Department deactivated successfully', department: updatedDepartment });
	} catch (error) {
		console.error('Error deleting department:', error);
		return res.status(500).json({ error: 'Internal server error while deleting department' });
	}
};

// ============ ROUTES ============
router.put('/:dept_id', requireModulePermission('departments', 'can_delete'), deleteDepartment);
