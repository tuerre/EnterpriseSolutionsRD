...
export default function Navbar({ onToggleSidebar, onToggleTheme, theme }) {
  ...
  const avatarLabel = (username.slice(0, 2) || 'U').toUpperCase();
  ...
  return (
    <header style={{ height: 56, background: 'var(--surface)', borderBottom: '1px solid var(--border)', marginLeft: 'var(--sidebar-width, 248px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', position: 'sticky', top: 0, zIndex: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text)', fontFamily: 'Cormorant Garamond, serif', fontSize: 16, flex: '0 0 auto' }}>
            {avatarLabel}
          </div>
          <div style={{ minWidth: 0, display: 'flex', flexDirection: 'column', lineHeight: 1.1 }}>
            <span style={{ color: 'var(--text)', fontSize: 13, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{displayName}</span>
            <span style={{ color: 'var(--text2)', fontSize: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{username}</span>
          </div>
        </div>
      </div>
    </header>
